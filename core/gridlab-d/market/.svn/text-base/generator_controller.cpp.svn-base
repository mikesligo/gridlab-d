	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_enumeration,"amortization_type", PADDR(amort_type), PT_DESCRIPTION, "Amortization compounding method",
				...
				PT_KEYWORD, "LINEAR_COST", LINEAR_COST,
				PT_KEYWORD, "LINEAR_BID", LINEAR_BID,
			...
			PT_double,"amortization_factor[1/s]",PADDR(amortization_value), PT_DESCRIPTION, "Decay factor in 1/s for shutdown cost repayment",
...
TIMESTAMP generator_controller::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	int index;
	int64 bidID_val;
	double per_phase_power, per_phase_old_power, shutdown_cost_temp, prev_section_power, prev_section_price;
