...
int eventgen::init(OBJECT *parent)
{
	...
	if (target_group[0] == '\0')	//Empty group list implies that manual list mode is desired (programmer defined. i.e., I say so)
	{
		...
		for (index=0; index<UnreliableObjCount; index++)
		{
			...
			//Make sure it is long enough - if not, force it to be - occurs for both
			if (((double)(tempTime)) < glob_min_timestep)
			{
...
TIMESTAMP eventgen::gen_random_time(DISTTYPE rand_dist_type, double param_1, double param_2)
{
	...
	//Round it - if necessary - check for all timestamps (in case someone is stupid and puts negative time)
	if (dbl_random_time < glob_min_timestep) 
	{
...
int eventgen::add_unhandled_event(OBJECT *obj_to_fault, char *event_type, TIMESTAMP fail_time, TIMESTAMP rest_length, int implemented_fault, bool fault_state)
{
	...
	//Force rest_length to at least one timestep, if it isn't - check in all situations
	if (((double)(rest_length)) < glob_min_timestep)
	{
...
EXPORT TIMESTAMP sync_eventgen(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	...
	{
		gl_error("%s (eventgen:%d): %s", obj->name?obj->name:"unnamed", obj->id, error);
		return TS_INVALID; 
	} 
	catch (...) 
	{
		gl_error("%s (eventgen:%d): unknown exception", obj->name?obj->name:"unnamed", obj->id);
		return TS_INVALID;
	}
