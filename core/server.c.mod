...
void http_response(SOCKET fd)
{
	...
	{
        // This is only executed once, find out where the thread is stopping the other times (or SHOULD be stopping) - prob http_send
		...
		if (strncmp(uri,"/",1)==0 )
		{
			...
			http_send(http);
            output_verbose("\nGOT HERE BRO - after http_send\n");
            // Gets to here still working fine
		}
        output_verbose("\nBRO - while loop condition is %d",(int)(len=recv_data(fd,http->query,sizeof(http->query))));
	}
    output_verbose("\nGOT HERE BRO - after while loop\n");
}
